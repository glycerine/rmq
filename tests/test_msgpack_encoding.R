...
e=expected.output = as.raw(c(0x85,0xa4,0x42,0x6c,0x6f,0x62,0xc4,0x03,0xff,0xf0,0x06,0xa1,0x44,0x92,0xa5,0x68,0x65,0x6c,0x6c,0x6f,0xa5,0x77,0x6f,0x72,0x6c,0x64,0xa1,0x45,0x92,0xcb,0x40,0x40,0x00,0x00,0x00,0x00,0x00,0x00,0xcb,0x40,0x31,0x00,0x00,0x00,0x00,0x00,0x00,0xa1,0x47,0x91,0xcb,0xc0,0x25,0x00,0x00,0x00,0x00,0x00,0x00,0xa3,0x53,0x75,0x62,0x83,0xa1,0x41,0x91,0xa2,0x68,0x69,0xa1,0x42,0x91,0xcb,0x43,0xd0,0x00,0x00,0x00,0x00,0x00,0x00,0xa1,0x46,0x92,0xcb,0x3f,0xf8,0x00,0x00,0x00,0x00,0x00,0x00,0xcb,0x40,0x0b,0x33,0x33,0x33,0x33,0x33,0x33))

o=observed.output = to.msgpack(input)
r=restored = from.msgpack(observed.output)

test_that("to.msgpack generates the expected encoded raw bytes", {    
    expect_equal(observed.output, expected.output)
  })

# save this test in case we have to back to non-canonicalized structures.
test_that("from.msgpack restores the same structures that to.msgpack encoded -- allowing variation in ordering of list/struct entries", {

  checker = function(restored, input) {
    rs=sort(names(restored))
    is=sort(names(input))
    expect_equal(rs, is)
    for (key in rs) {
      if (class(input[[key]])=="list") {
        checker(restored[[key]], input[[key]])
      } else {
        expect_equal(restored[[key]], input[[key]])
      }
    }
  }
  checker(restored, input)
  
  })

# now that we canonicalize (sort the maps by keys when we make the R structures), this
# test should pass. And it checks that canonicalization continues to work.
